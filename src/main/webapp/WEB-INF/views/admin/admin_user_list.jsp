...
<script>
	...
	function changePage(page) {
		...
		if (categoryNo) {
			...
			url = "/ticket/admin/member/listUsers?page=" + page;
		}
	    $.ajax({
	        type: 'GET',
	        url: url,
		...
		<c:forEach var="user" items="${usersList}">     
			<div id="cs-board-list-row">
                <div class="cs-board-list-row-box">
                    <input type="checkbox" name="selectedNotices" value="${user.user_no}">
                </div>
				<div class="cs-board-list-row-box">
					${user.u_name}
				</div>
				<div class="cs-board-list-row-box">
            		<a href="#" class="notice-link" data-notice-id="${user.user_no}">${user.u_id}</a>
        		</div>
				<div class="cs-board-list-row-box">
					${user.u_join_date}
				</div>
